       ...
       ORDER BY id DESC LIMIT 1`,
        ...
        if (existingTx && invoiceId) {
            try {
                ...
                // Якщо інвойс ще живий у Mono — повертаємо його ж
                if (monoStatus.status === 'created' || monoStatus.status === 'processing') {
                    return res.status(200).json({
                        pageUrl: monoStatus.pageUrl,
                        invoiceId,
                        amount: Number(existingTx.amount),
                    });
                }
            } catch {
                // Якщо сталася помилка перевірки старого інвойсу — створюємо новий нижче
            }
        }
...
export const handleMonoWebhook = async (req, res, next) => {
    try {
       ...
       LIMIT 1`,
        ...
        if (!tx) {
            console.warn(`⚠️ [MonoPay Webhook] Transaction not found for invoiceId: ${invoiceId}`);
        ...
        if (status === 'success' || status === 'hold') {
            ...
            if (currentShiftId) {
                ...
                if (targetWorkerId) {
                    await sequelize.query(
                        ...
                        { replacements: { shiftId: currentShiftId, workerId: targetWorkerId } }
                    );
                } else {
                    await sequelize.query(
                        `UPDATE shift_applications 
                ...
                console.log(`✅ [MonoPay Webhook] Handled invoice #${invoiceId} for shift #${currentShiftId}!`);
            }
            ...
            console.log(`❌ [MonoPay Webhook] Payment ${status}: ${failureReason || ''}`);
        }
...
export const verifyShiftInvoice = async (req, res, next) => {
    try {
       ...
       ORDER BY id DESC LIMIT 1`,
        ...
        const isPaid = monoStatus?.status === 'success' || monoStatus?.status === 'hold';
        const isFailed = monoStatus?.status === 'failure' || monoStatus?.status === 'reversed' || monoStatus?.status === 'expired';

        if (isPaid) {
            await sequelize.query(`UPDATE transactions SET status = 'completed' WHERE id = :id`, {
                replacements: { id: tx.id },
            });

            await sequelize.query(`UPDATE shifts SET status = 'booked' WHERE id = :shiftId`, {
                replacements: { shiftId },
            });
        ...
        if (isFailed) {
            await sequelize.query(`UPDATE transactions SET status = 'failed' WHERE id = :id`, {
                replacements: { id: tx.id },
            });
