...
async function requestMono(endpoint, body) {
  ...
  if (!response.ok) {
    ...
    throw new Error(data.errText || data.message || 'Помилка звернення до API Monobank');
  }
...
export const monopayService = {
   ...
   */
  async createInvoice({ amount, shiftId, applicationId, redirectUrl, webhookUrl }) {
    const hookUrl = webhookUrl || `${process.env.BACKEND_URL || 'http://localhost:5000'}/api/payments/webhook`;
    ...
    const payload = {
      ...
      redirectUrl: redirectUrl || `${process.env.CLIENT_URL || 'http://localhost:5173'}/dashboard`,
      webHookUrl: hookUrl,
      validity: 3600,
      paymentType: 'hold',
    };
  ...
  async getInvoiceStatus(invoiceId) {
    const response = await fetch(`${MONO_API_URL}/invoice/status?invoiceId=${invoiceId}`, {
      method: 'GET',
      headers: {
        'X-Token': MONO_TOKEN || '',
      },
