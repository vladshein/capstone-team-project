...
function BusinessDashboardPage() {
  ...
  useEffect(() => {
    ...
    const requestedCompany = companies.find(
      (company) => company.id === navigationState.companyId,
    );
    ...
    navigationState?.openCreateShift,
  ]);
  ...
  useEffect(() => {
    ...
    void getPendingBusinessShiftApplicationsCount(activeCompany.id)
      .then((count) => {
        if (!cancelled) setPendingApplicationsCount(count);
      })
      .catch(() => {
      ...
      <section className="mx-auto max-w-7xl px-4 py-[var(--space-section)] sm:px-6 md:px-8">
        <div className="rounded-[var(--radius-card)] border border-border bg-bg p-8 text-center shadow-sm">
          ...
          <p className="mx-auto mt-2 max-w-md text-sm text-text-muted">
            Спершу створіть профіль компанії — тоді зможете публікувати зміни й
            отримувати заявки.
          </p>
          <Link
            to="/profile"
    ...
    <section className="mx-auto max-w-7xl px-4 py-[var(--space-section)] sm:px-6 md:px-8">
          ...
          <div className="mt-3 flex flex-wrap items-end gap-x-3 gap-y-2 text-sm text-text-muted">
            {companies.length === 1 ? (
              <div className="flex flex-col">
                ...
                <span className="mt-0.5 font-medium text-text">
                  {activeCompany.name}
                </span>
              </div>
            ) : (
              <label className="flex cursor-pointer flex-col">
                ...
                <span className="relative mt-0.5 inline-flex items-center">
                  <select
                    ...
                    onChange={(event) =>
                      setActiveCompanyId(Number(event.target.value))
                    }
      ...
      <div className="overflow-hidden rounded-[var(--radius-card)] border border-border bg-bg shadow-sm">
        <nav
          ...
          aria-label="Розділи кабінету компанії"
        >
          {tabs.map(({ to, label, Icon }) => (
            <NavLink
              ...
              to={to}
              className={({ isActive }) =>
                `flex shrink-0 items-center gap-2 border-b-2 px-4 py-4 text-sm font-medium transition-colors sm:px-5 ${to === "statistics" ? "ml-auto" : ""} ${isActive ? "border-accent text-accent" : "border-transparent text-text-muted hover:text-text"}`
              }
            >
