...
export default function ShiftsDetailPage() {
  ...
  useEffect(() => {
    const isOwner =
      ...
      shift?.Location?.Company?.ownerId === user.id;
    ...
    void getBusinessShiftWorkerSummary(shift.id)
      .then((summary) => {
        if (!cancelled) setWorkerSummary(summary);
      })
      .catch(() => {
        if (!cancelled) setWorkerSummary(null);
      });
    return () => {
      cancelled = true;
    };
  }, [
    shift?.Location?.Company?.ownerId,
    shift?.endTime,
    shift?.id,
    user?.id,
    user?.role,
  ...
  const address =
    `${shift.Location?.address ?? "Адреса уточнюється"}, ${shift.Location?.city ?? ""}`.replace(
      /, $/,
      "",
    );
  ...
  const canManageShift =
    isShiftOwner && shift.status === "open" && !isShiftStarted;
  const canRepeatShift =
    isShiftOwner &&
  ...
  const handleApply = async () => {
    ...
    try {
      const createdApplication = await dispatch(
        applyToShift(shift.id),
      ).unwrap();
      setActiveApplication({
        ...createdApplication,
        status:
          createdApplication.status === "approved" ? "approved" : "pending",
        Shift: {
          id: shift.id,
  ...
  const handleEditShift = async (
    payload: Parameters<typeof updateShift>[1],
  ) => {
  ...
  const handleReviewSubmit = async ({
    ...
    setReviewError(null);
    try {
      if (workerSummary.review)
        await updateReview(workerSummary.review.id, { rating, comment });
    ...
    <>
            ...
            <section className="rounded-[var(--radius-card)] border border-border bg-bg p-5 shadow-sm sm:p-8">
              <div className="flex flex-col gap-5 border-b border-border pb-6 sm:flex-row sm:items-start sm:justify-between">
                <div className="flex gap-4">
                  ...
                  <div>
                    <Link
                      ...
                      className="text-sm text-text-muted transition-colors hover:text-accent-text hover:underline"
                    >
                      {companyName}
                    </Link>
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`inline-flex w-fit items-center gap-1.5 text-sm ${status.className}`}
                  >
                  ...
                  <button
                    ...
                    aria-label={
                      favorite
                        ? "Прибрати зміну з обраного"
                        : "Додати зміну в обране"
                    }
                  ...
                  >
                    <Heart
                      className={`h-5 w-5 ${favorite ? "fill-current" : "fill-none"}`}
                    />
                  </button>
                </div>
              </div>
              ...
              <div className="grid gap-3 py-6 sm:grid-cols-2">
                <div className="flex gap-3 rounded-[var(--radius-card)] bg-bg-muted p-4">
                  ...
                  <div>
                    <p className="text-xs text-text-muted">Дата зміни</p>
                ...
                <div className="flex gap-3 rounded-[var(--radius-card)] bg-bg-muted p-4">
                  ...
                  <div>
                    <p className="text-xs text-text-muted">Час роботи</p>
                ...
                <div className="flex gap-3 rounded-[var(--radius-card)] bg-bg-muted p-4 sm:col-span-2">
                  ...
                  <div>
                    <p className="text-xs text-text-muted">Локація</p>
                    <p className="mt-1 text-sm font-semibold text-ink">
                      {shift.Location?.title ?? companyName}
                    </p>
              ...
              <div className="border-t border-border pt-6">
                ...
                <p className="mt-3 whitespace-pre-line text-sm leading-6 text-text">
                  {shift.description ??
                    "Деталі завдань уточнюйте у представника компанії після відгуку."}
                </p>
              </div>
              ...
              <div className="mt-8 border-t border-border pt-6">
                ...
                <ul className="mt-4 grid gap-3 text-sm text-text sm:grid-cols-2">
                  <li className="flex items-center gap-2">
                    <Moon className="h-4 w-4 text-text-subtle" /> Тривалість за
                    графіком зміни
                  </li>
                  <li className="flex items-center gap-2">
                    <WalletCards className="h-4 w-4 text-text-subtle" /> Оплата
                    після виконання
                  </li>
                  <li className="flex items-center gap-2">
                  ...
                  {hasBonus && (
                    <li className="flex items-center gap-2">
                      ...
                      Бонус {moneyFormatter.format(bonusRate)} ₴
                    </li>
                  )}
                </ul>
              </div>
            ...
            <aside className="lg:sticky lg:top-24">
              <div className="rounded-[var(--radius-card)] border border-border bg-bg p-5 shadow-sm sm:p-6">
                ...
                <p className="mt-1 text-xs text-text-subtle">
                  {moneyFormatter.format(hourlyRate)} ₴/год{" "}
                  {hasBonus
                    ? `+ бонус ${moneyFormatter.format(bonusRate)} ₴`
                    : ""}
                </p>
                ...
                {canManageShift ? (
                  ...
                  <>
                    ...
                    <p className="mt-3 text-center text-xs leading-5 text-text-subtle">
                      Після відгуку компанія підтвердить вашу участь.
                    </p>
                    {applicationError && (
                ...
                {canOpenDispute && (
                  <button
                    ...
                    className="mt-3 flex min-h-[44px] w-full items-center justify-center rounded-[var(--radius-pill)] border border-danger/30 px-5 text-sm font-semibold text-danger transition-colors hover:bg-danger/10"
                  >
                    Відкрити спір
                  </button>
                )}
                {isShiftOwner &&
                  ...
                  (() => {
                    const profile =
                      workerSummary.application.User.WorkerProfile;
                    const workerName = profile
                      ? `${profile.firstName} ${profile.lastName}`
                      : "Виконавець";
                    const avatar =
                      profile?.avatarUrl ??
                      workerSummary.application.User.avatar;
                    const isNoShow =
                      workerSummary.application.status === "no_show";
                    return (
                      <div className="mt-5 border-t border-border pt-5">
                        <p className="text-xs font-medium uppercase tracking-wide text-text-subtle">
                          {isNoShow ? "Статус виконавця" : "Виконавець зміни"}
                        </p>
                        <div className="mt-3 flex items-center gap-3">
                          {avatar ? (
                            ...
                            <span className="flex h-11 w-11 items-center justify-center rounded-full bg-accent/10 font-heading font-semibold text-accent">
                              {workerName.charAt(0)}
                            </span>
                          )}
                          <div className="min-w-0">
                            <Link
                              to={`/workers/${workerSummary.application.User.id}`}
                              className="block truncate text-sm font-semibold text-ink transition-colors hover:text-accent-text hover:underline"
                            >
                              {workerName}
                            </Link>
                            <p className="mt-0.5 text-xs text-text-muted">
                              {isNoShow
                                ? "Не з’явився на зміну"
                        ...
                        <button
                          ...
                          className="mt-4 flex min-h-[42px] w-full items-center justify-center rounded-[var(--radius-pill)] border border-border px-4 text-sm font-medium text-text transition-colors hover:border-accent hover:text-accent-text"
                        >
                          {workerSummary.review
                            ? "Редагувати відгук"
                            : "Залишити відгук"}
                        </button>
                      </div>
      ...
      {isShiftOwner && (
        <CreateShiftModal
          ...
          onClose={() => {
            if (!isSavingBusinessShift) setIsEditingShift(false);
          }}
      ...
      <ReviewModal
        ...
        onClose={() => {
          if (!isSubmittingReview) setIsReviewModalOpen(false);
        }}
        title={
          workerSummary?.application.status === "no_show"
            ? "Відгук про неявку"
        ...
        description={
          workerSummary?.application.status === "no_show"
            ? "Залиште відгук, щоб інші компанії бачили історію співпраці."
            : "Як пройшла зміна з виконавцем?"
        }
      ...
      {isShiftOwner && (
        <CreateShiftModal
          ...
          onClose={() => {
            if (!isRepeatingShift) setIsRepeatModalOpen(false);
          }}
      ...
      <Modal
        ...
        onClose={() => {
          if (!isCancellingBusinessShift) setIsBusinessCancelModalOpen(false);
        }}
        title="Скасувати зміну?"
      >
        <p className="text-sm leading-6 text-text-muted">
          Усі заявки виконавців на цю зміну буде відхилено. Повернути зміну
          після скасування не вийде.
        </p>
        <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          <button
            type="button"
            onClick={() => setIsBusinessCancelModalOpen(false)}
            disabled={isCancellingBusinessShift}
            className="min-h-[44px] rounded-[var(--radius-pill)] border border-border px-5 text-sm font-semibold text-text transition-colors hover:border-accent"
          >
            Не скасовувати
          </button>
          <button
            ...
            disabled={isCancellingBusinessShift}
            className="min-h-[44px] rounded-[var(--radius-pill)] bg-danger px-5 text-sm font-semibold text-white transition-colors hover:opacity-90 disabled:opacity-60"
          >
