...
/**
 * One selectable metric on the chart. `getSecondary` (with its legend) is
 * only set for a stacked metric like shift count (completed + no-show); a
 * plain metric (hours, money) only defines `getPrimary` and renders a single
 * full-height bar.
 */
export interface PeriodMetric<TPoint extends PeriodPoint> {
  key: string;
  /** Long label shown above the chart, e.g. "Кількість змін". */
  label: string;
  /** Short label shown on the metric toggle button, e.g. "Зміни". */
  buttonLabel: string;
  getPrimary: (point: TPoint) => number;
  primaryColorClass?: string;
  getSecondary?: (point: TPoint) => number;
...
interface PeriodBarChartProps<TPoint extends PeriodPoint> {
  ...
  onRetry: () => void;
  metrics: PeriodMetric<TPoint>[];
}

type ChartPlaceholder = { period: string; isPlaceholder: true };
type ChartPoint<TPoint extends PeriodPoint> = TPoint | ChartPlaceholder;
...
/**
 * Metric-agnostic period bar chart: week/month toggle, a metric toggle
 * (whatever `metrics` the caller supplies), a stacked or single bar per
 * period, hover tooltips, and short-series left-padding with placeholder
...
export function PeriodBarChart<TPoint extends PeriodPoint>({
  ...
  const chartData = useMemo(() => {
    ...
    const rawMax = Math.max(...series.map(getValue));
    const maxValue = niceMax(rawMax);

    const MIN_SLOTS = 6;
    const missing = Math.max(0, MIN_SLOTS - series.length);

    const placeholders: ChartPlaceholder[] = [];
    let cursor = series[0].period;
    ...
    const points: ChartPoint<TPoint>[] = [...placeholders, ...series];

    return { points, maxValue };
  }, [series, activeMetric, groupBy]);
        ...
        <div className="flex gap-1 rounded-md border border-border p-1">
          {metrics.map((m) => (
            <button
              ...
              className={`rounded px-3 py-1 text-xs font-medium ${
                metricKey === m.key ? "bg-accent text-white" : "text-text-muted hover:bg-bg-muted"
              }`}
            >
              {m.buttonLabel}
            </button>
                ...
                <div className="relative h-40 sm:h-52 md:h-56 min-w-full">
                  ...
                  <div className="relative flex h-full items-end gap-1.5 sm:gap-2">
                    {chartData.points.map((p, i) => {
                      ...
                      const primary = activeMetric.getPrimary(p);
                      const secondary = activeMetric.getSecondary?.(p) ?? 0;
                      const value = primary + secondary;
                      const primaryPct = (primary / chartData.maxValue) * 100;
                      const secondaryPct = (secondary / chartData.maxValue) * 100;
                        ...
                        >
                          ...
                          {activeMetric.getSecondary && (
                            <div
                              ...
                              style={{ height: `${secondaryPct}%`, marginBottom: 2, minHeight: secondary > 0 ? 2 : 0 }}
                            />
                          )}
                          <div
                            className={`w-full rounded-t-sm ${activeMetric.primaryColorClass ?? "bg-accent"}`}
                            style={{ height: `${primaryPct}%`, minHeight: value > 0 ? 2 : 0 }}
                          />
            ...
            {activeMetric.legend && (
              <div className="mt-4 flex items-center gap-4 text-xs text-text-muted">
                <span className="flex items-center gap-1.5">
                  <span className={`h-2.5 w-2.5 rounded-sm ${activeMetric.primaryColorClass ?? "bg-accent"}`} />
                  {activeMetric.legend.primaryLabel}
                </span>
                <span className="flex items-center gap-1.5">
                  <span className={`h-2.5 w-2.5 rounded-sm ${activeMetric.secondaryColorClass ?? "bg-danger"}`} />
                  {activeMetric.legend.secondaryLabel}
                </span>
              </div>
            )}
          </div>
